...
import GET_MY_TEXTBOOKS from "./getMyTextbooks.graphql";
...
test("should return the created textbook correctly with no query", (done) => {
  gql({
    query: GET_MY_TEXTBOOKS
  })
    .then((returnedTextbooks) => {
      expect(returnedTextbooks).toHaveLength(textbooks.length);
    })
    .then(() => done())
    .catch(done);
});

test("should return the created textbook correctly with query as a string", (done) => {
  gql({
    query: GET_MY_TEXTBOOKS,
    variables: {
      query: "ISBN13"
    ...
    .then((returnedTextbooks) => {
      expect(returnedTextbooks).toHaveLength(1);
    })
    .then(() => done())
    .catch(done);
});

test("should return the created textbook correctly with query as a ISBN", (done) => {
  gql({
    query: GET_MY_TEXTBOOKS,
    variables: {
...
test("should return the created textbook correctly with limit and offset", (done) => {
  gql({
    query: GET_MY_TEXTBOOKS,
    variables: {
      limit: 1,
